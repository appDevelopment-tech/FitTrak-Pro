...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Wger API proxy endpoints to bypass CORS
  app.get("/api/wger/exercises", async (req, res) => {
    try {
      const { language = 2, limit = 50 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/exercise/?language=${language}&limit=${limit}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger exercises error:', error);
      res.status(500).json({ error: "Failed to fetch exercises from Wger API" });
    }
  });

  app.get("/api/wger/exerciseinfo", async (req, res) => {
    try {
      const { language = 2, limit = 50 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/exerciseinfo/?language=${language}&limit=${limit}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger exercise info error:', error);
      res.status(500).json({ error: "Failed to fetch exercise info from Wger API" });
    }
  });

  app.get("/api/wger/categories", async (req, res) => {
    try {
      const { language = 2 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/exercisecategory/?language=${language}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger categories error:', error);
      res.status(500).json({ error: "Failed to fetch categories from Wger API" });
    }
  });

  app.get("/api/wger/muscles", async (req, res) => {
    try {
      const { language = 2 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/muscle/?language=${language}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger muscles error:', error);
      res.status(500).json({ error: "Failed to fetch muscles from Wger API" });
    }
  });

  app.get("/api/wger/equipment", async (req, res) => {
    try {
      const { language = 2 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/equipment/?language=${language}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger equipment error:', error);
      res.status(500).json({ error: "Failed to fetch equipment from Wger API" });
    }
  });

  app.get("/api/wger/images", async (req, res) => {
    try {
      const { limit = 100 } = req.query;
      const response = await fetch(`https://wger.de/api/v2/exerciseimage/?limit=${limit}`);
      if (!response.ok) throw new Error('Wger API error');
      const data = await response.json();
      res.json(data);
    } catch (error) {
      console.error('Wger images error:', error);
      res.status(500).json({ error: "Failed to fetch images from Wger API" });
    }
  });
